...
const Card = ({item, pathDemo, pathGitHub }) => {
  ...
  return (
    <div className="relative shadow-lg shadow-bg2 group rounded-xl max-w-[250px] min-h-[350px] flex-[45%] mb-12 md:mx-2">
      <div style={{backgroundImage: `url(${image})`, backgroundPosition: 'top', backgroundSize:'cover', borderRadius:'10px'}} className='h-[350px]'>
      ...
      {/* hover */}
        <div className='opacity-0 group-hover:opacity-100 transition-all ease-in duration-300 p-8 h-full flex flex-col items-center justify-center z-10'>
          ...
          {title !== 'Portfolio' && (
            <a href={pathDemo} target="_blank" rel="noreferrer" className='hover:text-primary transition-colors duration-300 ease-in'>
              <FaEye size={32}/>
            </a>
          )}
            <a href={pathGitHub} target="_blank" rel="noreferrer" className='hover:text-primary transition-colors duration-300 ease-in'>
              <FaGithub size={32}/>
            </a>
          </div>
        </div>
